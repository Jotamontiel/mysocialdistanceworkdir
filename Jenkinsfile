    ...
    withCredentials([sshUserPrivateKey(credentialsId: 'the-real-user-for-webserver-1', keyFileVariable: 'WEBSERVER_KEYFILE', passphraseVariable: '', usernameVariable: 'WEBSERVER_USER')]) {
        ...
        stage("Stage 10: Create Users"){
            withCredentials([usernamePassword(credentialsId: 'the-real-user-password-for-BD-webserver-1', passwordVariable: 'JENKINS_BD_PASS', usernameVariable: 'JENKINS_BD_USER')]) {
                echo "----------------------------------------------------------------------"
                echo "---------------- Jorge User Creation Starting ------------------------"
                echo "----------------------------------------------------------------------"
                sshCommand remote: remote, command: """
                cd mysocialdistanceworkdir
                source mysocialdistanceworkenv/bin/activate
                ...
                """
                echo "----------------------------------------------------------------------"
                echo "---------------- Jorge User created ----------------------------------"
                echo "----------------------------------------------------------------------"
            }
            withCredentials([usernamePassword(credentialsId: 'the-real-user-password-N2-for-BD-webserver-1', passwordVariable: 'JENKINS_BD_PASS_2', usernameVariable: 'JENKINS_BD_USER_2')]) {
                echo "----------------------------------------------------------------------"
                echo "---------------- Fernando User Creation Starting ---------------------"
                echo "----------------------------------------------------------------------"
                sshCommand remote: remote, command: """
                cd mysocialdistanceworkdir
                source mysocialdistanceworkenv/bin/activate 
                echo "from django.contrib.auth.models import User; User.objects.create_user('$JENKINS_BD_USER_2', 'fernandoperezorrego@gmail.com', '$JENKINS_BD_PASS_2')" | python manage.py shell
                """
                echo "----------------------------------------------------------------------"
                echo "---------------- Fernando User created -------------------------------"
                echo "----------------------------------------------------------------------"
            }
        }
...
// 6°) Create the package directory with the next name "myprojectpackage" in the path: home/jorge/, then you can create the next subdirectories:
// 6.1) Prepare the "full_database_bkp" subdirectory, contains the "myprojectdb_bkp.pgsql" file with the full data base backup
// 6.2) Prepare the "prod_mediafiles_bkp" subdirectory, contains the "media" directory with all media records and media files from the django platform
// 6.3) Prepare the "prod_tables_bkp" subdirectory, contains the "table_bkp.sql" files for each table than you need make a backup
// 6.4) Prepare the "sftp_files_landing", contains the "nasdaqlisted.txt" and "otherlisted.txt" files extracted from a SFTP for the liketrading project
// 6.5) Prepare the "trigger_queries" subdirectory, contains the triggers executables files than generated by the aforementioned files, the scrips are "db_reset_query.sql", "full_database_bkp_script", "prod_tables_bkp_script", "repopulate_database_script"
// 6.5.1) Prepare the "db_reset_query.sql" file with the next:
//
//  COMMAND: sudo touch db_reset_query.sql
//  COMMAND: sudo nano db_reset_query.sql
//
// ********** /home/jorge/myprojectpackage/trigger_queries/db_reset_query.sql **********
// DROP DATABASE myprojectdb;
// CREATE DATABASE myprojectdb;
// GRANT ALL PRIVILEGIES ON DATABASE myprojectdb TO <db_user>;
//
// 6.5.2) Prepare the "full_database_bkp_script" file with the next:
//
//  COMMAND: sudo touch full_database_bkp_script
//  COMMAND: sudo chmod +x full_database_bkp_script
//  COMMAND: sudo nano full_database_bkp_script
//
// ********** /home/jorge/myprojectpackage/trigger_queries/full_database_bkp_script **********
// #!/bin/bash
// 
// PGPASSWORD="<db_password>" pg_dump <myprojectdb> -U <db_user> -h localhost > /home/jorge/myprojectpackage/full_database_bkp/myprojectdb_bkp.pgsql
//
// 6.5.3) Prepare the "prod_tables_bkp_script" file with the next code:
//
//  COMMAND: sudo touch prod_tables_bkp_script
//  COMMAND: sudo chmod +x prod_tables_bkp_script
//  COMMAND: sudo nano prod_tables_bkp_script
//
// ********** /home/jorge/myprojectpackage/trigger_queries/prod_tables_bkp_script **********
// #!/bin/bash
// 
// PGPASSWORD="<db_password>" pg_dump <myprojectdb> -U <db_user> -h localhost -a -b -t <table_name> > /home/jorge/myprojectpackage/prod_tables_bkp/<table_name>_bkp.sql
// PGPASSWORD="<db_password>" pg_dump <myprojectdb> -U <db_user> -h localhost -a -b -t <table_name> > /home/jorge/myprojectpackage/prod_tables_bkp/<table_name>_bkp.sql
// PGPASSWORD="<db_password>" pg_dump <myprojectdb> -U <db_user> -h localhost -a -b -t <table_name> > /home/jorge/myprojectpackage/prod_tables_bkp/<table_name>_bkp.sql
// .
// .
// .
//
// 6.5.4) Prepare the "repopulate_database_script" file with the next code:
//
//  COMMAND: sudo touch repopulate_database_script
//  COMMAND: sudo chmod +x repopulate_database_script
//  COMMAND: sudo nano repopulate_database_script
//
// ********** /home/jorge/myprojectpackage/trigger_queries/repopulate_database_script **********
// #!/bin/bash
// 
// export PGPASSWORD="<db_password>"
// psql -U <db_user> -w -h localhost <myprojectdb> < /home/jorge/myprojectpackage/prod_tables_bkp/<table_name>_bkp.sql
// psql -U <db_user> -w -h localhost <myprojectdb> < /home/jorge/myprojectpackage/prod_tables_bkp/<table_name>_bkp.sql
// psql -U <db_user> -w -h localhost <myprojectdb> < /home/jorge/myprojectpackage/prod_tables_bkp/<table_name>_bkp.sql
// .
// .
...
//
//  COMMAND: celery -A nombre_proyecto worker --autoscale=10,0 --loglevel=INFO    #Linux   Version
//  COMMAND: celery -A nombre_proyecto worker --pool=solo -l info                 ##Windows Version
//
...
// ExecStart=/bin/sh -c '${CELERY_BIN} -A ${CELERY_APP} ${CELERYD_NODES} --autoscale=10,0 --pidfile=${CELERYD_PID_FILE} --logfile=${CELERYD_LOG_FILE} --loglevel=${CELERYD_LOG_LEVEL}'
// Restart=on-failure
//
// [Install]
// WantedBy=multi-user.target
